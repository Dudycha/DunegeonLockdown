...
public class Pistol1911 extends Weapon {
    ...
    private final int MAGAZINECAPACITY = 7;
    private int ammoInMagazine = 7;
    private int reloadClock = 0;
    private boolean reloading = false;

    public Pistol1911(Handler handler) {
        super(handler);
        ammoInReserve = 35;
    }
    ...
    public void renderWeaponPictureAndAmmo(Graphics g) {
        ...
        for (int i = 0; i < 2; i++) {
            g.drawImage(Assets.pistol1911MainPicture[i], 280 + (i * shiftForWeapon), 507, null);
        }
        ...
        if (reloading) {
            ...
            if (ammoInMagazine > 0) {
                for (int i = 0; i < ammoInMagazine; i++) {
                    g.drawImage(Assets.bullet9mm, 470 + i * shiftForAmmo, 507, null);
                }
        ...
        if (mouseX == 0 || mouseY == 0 || playerX == 0 || playerY == 0 || (mouseY - playerY == 0)) {
            ...
            if (mouseX > playerX && mouseY < playerY) {            // PRAVA HORNI SEKCE
                if ((mouseX - playerX) / (playerY - mouseY) < 1) {
                    g.drawImage(Assets.pistolUp, playerX, playerY, null);
                } else if ((mouseX - playerX) / (playerY - mouseY) > 5) {
                    g.drawImage(Assets.pistolRight, playerX, playerY, null);
                } else {
                    g.drawImage(Assets.pistolUpRight, playerX, playerY, null);
                }
            }

            if (mouseX > playerX && mouseY > playerY) {           // PRAVA DOLNI SEKCE
                if ((mouseX - playerX) / (playerY - mouseY) > -1) {
                    g.drawImage(Assets.pistolDown, playerX, playerY, null);
                } else if ((mouseX - playerX) / (playerY - mouseY) < -5) {
                    g.drawImage(Assets.pistolRight, playerX, playerY, null);
                } else {
                    g.drawImage(Assets.pistolDownRight, playerX, playerY, null);
                }
            }
            ...
            if (mouseX < playerX && mouseY > playerY) {         // LEVA DOLNI SEKCE
                if ((mouseX - playerX) / (playerY - mouseY) < 1) {
                    ...
                    g.drawImage(Assets.pistolLeft, playerX, playerY, null);
                } else {
                    g.drawImage(Assets.pistolDownLeft, playerX, playerY, null);
                }
            }
            ...
            if (mouseX < playerX && mouseY < playerY) {         // LEVA HORNI SEKCE
                if ((mouseX - playerX) / (playerY - mouseY) > -1) {
                    ...
                    g.drawImage(Assets.pistolUpLeft, playerX, playerY, null);
                }
            }
        }
