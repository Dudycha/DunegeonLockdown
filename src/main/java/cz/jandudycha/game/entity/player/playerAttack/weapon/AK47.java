...
public class AK47 extends Weapon {
    ...
    private void checkForHit() {
        ...
        for (Enemy enemy : getHandler().getGame().getEvents().getListOfEnemies()) {
            position++;
            if (enemy.isAlive()) {
                if (lineLineIntersection(enemy.getHitVectorsOnPosition(0), enemy.getHitVectorsOnPosition(1), enemy.getHitVectorsOnPosition(2),
                        enemy.getHitVectorsOnPosition(3), drawFireLineFromX, drawFireLineFromY, drawFireLineToX, drawFireLineToY) ||
                        (lineLineIntersection(enemy.getHitVectorsOnPosition(4), enemy.getHitVectorsOnPosition(5), enemy.getHitVectorsOnPosition(6),
                                enemy.getHitVectorsOnPosition(7), drawFireLineFromX, drawFireLineFromY, drawFireLineToX, drawFireLineToY))) {
    ...
    private void extendLine() {
        ...
        drawFireLineFromX = playerX + 15;  //15
        drawFireLineFromY = playerY + 13;  //13
        ...
        if (mouseX == 0 || mouseY == 0 || playerX == 0 || playerY == 0 || (mouseY - playerY == 0)) {
            ...
            if (mouseX > playerX && mouseY < playerY) {            // PRAVA HORNI SEKCE
                if ((mouseX - playerX) / (playerY - mouseY) < 1) {
                    g.drawImage(Assets.AKUp, playerX, playerY, null);
                    setPositionOfGun(PositionOfGun.Up);
                } else if ((mouseX - playerX) / (playerY - mouseY) > 5) {
                    g.drawImage(Assets.AKRight, playerX, playerY, null);
                    setPositionOfGun(PositionOfGun.Right);
                } else {
                    g.drawImage(Assets.AKUpRight, playerX, playerY, null);
            ...
            if (mouseX > playerX && mouseY > playerY) {           // PRAVA DOLNI SEKCE
                if ((mouseX - playerX) / (playerY - mouseY) > -1) {
                    g.drawImage(Assets.AKDown, playerX, playerY, null);
                    setPositionOfGun(PositionOfGun.Down);
                } else if ((mouseX - playerX) / (playerY - mouseY) < -5) {
                    g.drawImage(Assets.AKRight, playerX, playerY, null);
                    setPositionOfGun(PositionOfGun.Right);
                } else {
                    g.drawImage(Assets.AKDownRight, playerX, playerY, null);
            ...
            if (mouseX < playerX && mouseY > playerY) {         // LEVA DOLNI SEKCE
                if ((mouseX - playerX) / (playerY - mouseY) < 1) {
                    ...
                    g.drawImage(Assets.AKLeft, playerX, playerY, null);
                    setPositionOfGun(PositionOfGun.Left);
                } else {
                    g.drawImage(Assets.AKDownLeft, playerX, playerY, null);
            ...
            if (mouseX < playerX && mouseY < playerY) {         // LEVA HORNI SEKCE
                if ((mouseX - playerX) / (playerY - mouseY) > -1) {
                    ...
                    g.drawImage(Assets.AKUpLeft, playerX, playerY, null);
    ...
    public void renderWeaponPictureAndAmmo(Graphics g) {
        ...
        for (int i = 0; i < 5; i++) {
            g.drawImage(Assets.AK_47MainPictureBig[i], 210 + (i * 30), yPosition, 30, 30, null);
            g.drawImage(Assets.AK_47MainPictureBig[i + 5], 210 + (i * 30), yPosition + 30, 32, 30, null);
        }
        ...
        if (reloading) {
            ...
            if (ammoInMagazine > 0) {
                for (int i = 0; i < ammoInMagazine; i++) {
                    g.drawImage(Assets.bullet556, 375 + i * shift, 502, 41, 41, null);
                }
